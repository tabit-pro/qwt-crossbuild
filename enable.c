...
DHPDEV APIENTRY DrvEnablePDEV(
    ...
    SAL__in_opt PWCHAR LogicalAddress,	// Logical address, for printer drivers
    __in ULONG NumberPatterns,	// Number of patterns, for printer drivers
    SAL__in_opt HSURF *StandardPatterns, // Return standard patterns, for printer drivers
    ...
    SAL__in_opt HDEV EngDeviceHandle,	// GDI-supplied handle to the device, used for callbacks
    SAL__in_opt WCHAR *DeviceName,	// User-readable device name
...
ULONG APIENTRY DrvGetModes(
    ...
    SAL__out_bcount_opt(cbDevMode) PDEVMODEW DevMode // display mode array
    )
{
